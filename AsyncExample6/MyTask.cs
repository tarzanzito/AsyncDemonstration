...
namespace AsyncExample6
{
    ...
    internal class MyTasks
    {
        ...
        public async Task<string> MyCounterRunAsync(int total, int pause)
        {
            Console.WriteLine($"    MyTask.MyCounterRunAsync: Before RUN: {total.ToString("00")}:{pause.ToString("0000")}");

            Task<string> task = Task<string>.Run(async () => await CounterAsync(total, pause));
        ...
        private async Task<string> CounterAsync(int total, int pause)
        {
